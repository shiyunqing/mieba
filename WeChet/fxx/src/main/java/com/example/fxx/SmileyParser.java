...
import com.example.fxx.anim.gif.AnimatedGifDrawable;
import com.example.fxx.anim.gif.AnimatedImageSpan;
...
//        	Log.w("SmileyParser", "Smiley resource ID/text mismatch");
     ...
     * @return
     */
    public CharSequence replace(final TextView txtView, CharSequence text, int faceType){
        SpannableStringBuilder builder = new SpannableStringBuilder(text);
        Matcher matcher = mPattern.matcher(text);
        while (matcher.find()) {
